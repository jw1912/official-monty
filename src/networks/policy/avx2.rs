...
pub unsafe fn l2(w: &Align64<i8, {L1 * DIM / 2}>, l1: &Align64<u8, {L1 / 2}>) -> Align64<i32, DIM> {
    ...
    const L2C: usize = DIM / CS;

    let l1 = &*ptr(l1).cast::<Align64<i32, {L1 / 8}>>();
    let l2p = ptr_mut(&mut l2).cast::<__m256i>();
    let ws = ptr(w).cast::<__m256i>();
    ...
    for i in (0..L1 / 8).step_by(2) {
        ...
        for j in 0..L2C {
            // QB
            let this = l2p.add(j);
            let wa = _mm256_load_si256(ws.add(4 * DIM * i / CSB + j));
            let wb = _mm256_load_si256(ws.add(4 * DIM * (i + 1) / CSB + j));
